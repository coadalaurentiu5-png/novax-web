...
if (cluster.isMaster) {
    ...
    // Notificări prin BOT Discord (mai stabil decât webhook pe IP-uri partajate)
    const DISCORD_BOT_TOKEN   = process.env.DISCORD_BOT_TOKEN   || '';   // token-ul botului
    const DISCORD_CHANNEL_ID  = process.env.DISCORD_CHANNEL_ID  || '';   // ID-ul canalului secret de staff
    ...
    // Trimite notificarea prin BOT Discord (folosind API-ul official de mesaje)
    async function sendDiscordStaffNotification(payload) {
        if (!DISCORD_BOT_TOKEN || !DISCORD_CHANNEL_ID) {
            console.log('[DISCORD] Bot token sau channel ID neconfigurat — notificarea NU a fost trimisă.');
            return false;
        }
        console.log(`[DISCORD] Se trimite notificarea prin bot către canalul ${DISCORD_CHANNEL_ID}...`);
        const url = `https://discord.com/api/v10/channels/${DISCORD_CHANNEL_ID}/messages`;
        const headers = {
            'Content-Type': 'application/json',
            'Authorization': `Bot ${DISCORD_BOT_TOKEN}`,
            'User-Agent': 'NovaxWebBot/1.0 (https://novax-web.onrender.com)',
        };
        ...
        // Reîncearcă de câteva ori cu pauză, în caz de 429 (rate limit)
        const maxAttempts = 5;
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                const resp = await fetch(url, { method: 'POST', headers, body });
                console.log(`[DISCORD] Încercare ${attempt}: status ${resp.status}`);
                if (resp.status === 204 || resp.status === 200) {
                    console.log('[DISCORD] ✅ Notificare trimisă cu succes pe Discord.');
                    return true;
                }
                if (resp.status === 429) {
                    const waitMs = 2000 * attempt;
                    console.log(`[DISCORD] Rate limit (429). Reîncerc în ${waitMs / 1000}s...`);
                    await new Promise(r => setTimeout(r, waitMs));
                    continue;
                }
                const txt = await resp.text();
                console.log(`[DISCORD] Eroare status ${resp.status}: ${txt.slice(0, 200)}`);
                // 401 = token invalid, 403 = bot fără permisiune
                return false;
            } catch (err) {
                console.error(`[DISCORD] Eroare de rețea (încercarea ${attempt}):`, err.message);
                await new Promise(r => setTimeout(r, 2000));
            }
        }
        console.log('[DISCORD] Nu am reușit să trimit notificarea după mai multe încercări.');
        return false;
    }
